...
class changeMainAdvisorOfClient(graphene.Mutation):
    ...
    def mutate(self, info, token, advisor_id, profile_id):
        ...
        if cpf:
            ...
            if not(ProfileAdvisors.objects.filter(
                    profile_id=profile_id, advisor_id=token_owner.id, main_advisor=True)):
...
class Mutation(graphene.ObjectType):
    ...
    change_main_advisor_from_client = changeMainAdvisorOfClient.Field(description="Altera o assesor principal do cliente")
